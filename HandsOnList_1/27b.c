...
//Q27b Write a program to execute ls -Rl using execlp

#include <unistd.h>//includes declarations for the execlp function 
#include <stdio.h>// includes declarations for perror

int main() {
    execlp("ls", "ls", "-Rl", (char *)NULL);
    //int execlp(const char *file, const char *arg0, ..., (char *)NULL);
    //execlp uses the PATH environment variable to locate the file if the provided path is not absolute
    // rest of the parameters are same as exec.
    perror("execlp failed");
    return 1;
}
/*Output:
kunalagarwal@Kunals-MacBook-Air software systems handson 1 % cc 27b.c
...
-rwxr-xr-x@ 1 kunalagarwal  staff  33480 Aug 31 15:36 a.out
